...
<div class="card">
    ...
    {#if system.owner_name}
      <div class="owner">
        ...
        {#if system.owner_telegram}
           <a href="https://t.me/{system.owner_telegram.replace('@', '')}" target="_blank">(@{system.owner_telegram.replace('@', '')})</a>
        {/if}
      </div>
    {/if}
  </div>
  ...
  <div class="links">
    {#if system.wiki_url} <a href={system.wiki_url} target="_blank" class="btn wiki">Wiki</a> {/if}
    {#if system.jira_url} <a href={system.jira_url} target="_blank" class="btn jira">Jira</a> {/if}
    {#if system.repo_url} <a href={system.repo_url} target="_blank" class="btn repo">Repo</a> {/if}
  </div>
</div>
...
<style>
  ...
  .header { display: flex; justify-content: space-between; align-items: start; margin-bottom: 0.5rem; }
  .title-group { display: flex; flex-direction: column; gap: 5px; }
  h3 { margin: 0; font-size: 1.25rem; color: #111827; }
  ...
  .score { background: #10b981; color: white; padding: 2px 8px; border-radius: 12px; font-size: 0.75rem; font-weight: bold; height: fit-content;}
  .description { color: #4b5563; margin-bottom: 1rem; }
  
  /* AI Section Styles */
  .ai-section { margin-bottom: 1rem; }
  .ai-btn {
    background: none; border: 1px solid #8b5cf6; color: #7c3aed;
    padding: 5px 10px; border-radius: 6px; cursor: pointer; font-size: 0.85rem;
    transition: all 0.2s;
  }
  .ai-btn:hover { background: #f5f3ff; }
  .summary-content {
    margin-top: 10px; padding: 10px; background: #f5f3ff;
  ...
  .btn { text-decoration: none; padding: 0.4rem 0.8rem; border-radius: 4px; font-size: 0.875rem; color: white; }
  .wiki { background-color: #0052CC; }
  .jira { background-color: #2684FF; }
  .repo { background-color: #24292e; }
</style>